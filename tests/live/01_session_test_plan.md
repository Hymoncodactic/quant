...
## 9. 操作模型变更（2026-08-31，覆盖 §3-§4 的手动命令）

常驻调度器上线后，T2/T3/T4 不再需要掐点手敲命令：

1. 看板「策略进程」面板点「启动策略」——任何时刻都可以点，进程自动等窗口。
2. 是否真实提交由风控设置的「模拟」开关决定，改动即刻生效（下一周期现读）。
3. 决策与结算自动发生；结果看「策略进程」面板的最近决策/结算行、
   「已决策场次」表与系统通知。
4. 停止：同面板「停止策略」。急停仍是急停按钮（立起旗标，调度器下一相位即止步）。

手动命令仅保留为调试与恢复手段（§3 原文），不再是常规操作路径。
