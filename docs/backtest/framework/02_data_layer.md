...
| 2026-08-21 | §7 数据质量闸：OHLC 次序检验加相对容差 1e-9（实测复权股票违规全部为 ~1e-16 的 1 ULP 浮点伪影，非数据错误；超容差仍然中止）。GBPUSD=X 读入时 H/L 包络到含 O/C（Yahoo FX 收盘取自不同截面，实测偏差 ≤6e-4，103 根；引擎只读 FX 收盘，该修复不改变任何数字）。依据：scripts/20260821_a0_framework_backtest.py 首跑的全池违规扫描 |
