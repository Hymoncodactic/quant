...
Responsibility: read Binance kline partitions and reduce them to the engine
bar schema (ts, open, high, low, close, volume, quote_ccy). The backtest
never touches a network (ARCHITECTURE.md section 2, backtest row).
Not responsible for: alignment or validation (engine/feed.py), OKX cost or
execution modeling (a future okx broker adapter; blocked on S4 fee research).

Data facts (docs/data/binance/DATA_SPEC.md; verified against the tree
2026-08-21): spot klines exist for 9 USDT pairs at 1d and 1m, 2017-08 to
present; columns ts/open/high/low/close/volume plus flow extras
(quote_volume, count, taker_buy_*). ts is the bar OPEN time, UTC. Binance is
a DATA SOURCE only, never a venue (common/paths.py DATA_SOURCES vs VENUES).
