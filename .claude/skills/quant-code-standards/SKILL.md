...
对应 `CLAUDE.md` §2.3 语言纪律。本节是该纪律在代码侧的落地口径。
...
| md 文档 | 中文 | `CLAUDE.md`、`ARCHITECTURE.md`、`WORKING_MEMORY.md`、`.claude/skills/*/SKILL.md`、`DATA_SPEC.md`、`research/`、`reports/` |
...
1. 代码文件内不出现中文字符，注释与 docstring 同样适用。唯一例外是必须原样保留的
   ...
   代码内允许出现的非 ASCII 字符只有章节引用符号 `§`（用于引用 `CLAUDE.md` 的条款号）。
...
对应 `CLAUDE.md` §4.4。模块头 docstring 是该文件的说明文档，作用等同于目录 `README.md`。
...
1. 代码注释与技术文档中不用 emoji 及装饰性符号（`CLAUDE.md` §2.1.2）。
...
| HIGH | 所在目录缺 `README.md`（`CLAUDE.md` §4.3） |
