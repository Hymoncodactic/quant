...
`docs/backtest/validation/02_test_plan.md` §2 的裁定放在本目录，不放 `tests/`
（`tests/README.md` §1）。
...
| `20260819_ingest_equity.py` | 一次性 ingest，首轮落地。为三组共 52 个标的下载 5 个区间（1d、1h、5m、2m、1m）的 Yahoo 行情，日线按自然年、日内按自然月分区落地；1m 因单请求上限 8 天而由连续窗口拼接 | `docs/data/t212/DATA_SPEC.md:8` 登记它为该数据集的生成脚本。**注意重复**：其取数与写盘逻辑此后已提炼为 `trading212/ingest/yahoo_bars.py`（该模块 docstring 自述为「单一实现，首轮与增量都调它」），本脚本内仍保有一份独立实现（`_fetch_interval`/`_write_daily`/`_write_intraday`/`_quote_currency` 与 `INTERVALS`/`UNIVERSE` 常量），两份构成重复定义，但属**有意冻结**：本脚本是首轮落地的留痕件，不随行为变更同步，改行为一律改 `yahoo_bars.py`。2026-08-22 核对 `INTERVALS` 与 `UNIVERSE` 两份仍完全一致，各 52 个标的 | `docs/data/t212/DATA_SPEC.md:8`。无代码调用点 |
...
| `20260821_a0_framework_backtest.py` | 一次性运行入口。加载 `trading212/strategy/a0_v0_0_1.py` 与基线参数 `trading212/config/strategies/a0_v0_0_1.yaml`，以显式覆盖派生 4 个消融臂（a0、tsmom、ma200、bh），每臂跑 2 档费率，跑 2010-01-04 至 2026-08-19（2018-01-01 起实盘化），并从成交记录后处理持仓时长与年化换手，写 `backtest/results/a0_comparison_20260821.csv` | `research/decisions/20260821_a0_framework_comparison.md:5` 指名它为该裁定的入口脚本，裁定中的全部数字由它产出；删除后裁定失去可复现凭据 | `research/decisions/20260821_a0_framework_comparison.md:5`；`docs/backtest/framework/02_data_layer.md:103` 引用其首跑的全池违规扫描；`tests/README.md:10` |
...
| `20260829_live_probe.py` | 常驻只读探针，六个子命令：`latency`（端点延迟分布，按场所计量分别设采样数）、`clock`（本机与场所时钟偏差，即 decide 的时钟闸读数）、`instruments`（标的池解析、计价、交易日历 id 一致性）、`costs`（从 `trading212/records/orders.jsonl` 提取真实每笔费用 bps，按市场与买卖方向拆分）、`rehearse`（用真实行情与真实账本走完整条决策链到干跑路由，不提交）、`drills`（急停、悬空意向冻结、撕裂 journal 三项演练）。**全文不含任何下单调用** | 交易时段窗口每日仅 29 分钟，成本与延迟这类指标必须有事先写好的提取口径才测得出；`tests/live/01_session_test_plan.md` 的 T1–T4 直接调它。同时它是每次实盘前的开机自检入口 | 人工执行；`tests/live/01_session_test_plan.md` 指名为驱动脚本 |
